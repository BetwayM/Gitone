...
class ViewController: UIViewController {
    ...
    let FullDistance: CGFloat = 0.78//右侧主视图距离屏幕左边界的距离占屏幕宽度的比例
    let Proportion: CGFloat = 0.77//右侧主视图高度占屏幕高度的比例
    var centerOfLeftViewAtBeginning: CGPoint!
     var centerOfRightViewAtBeginning: CGPoint!
    var proportionOfLeftView: CGFloat = 1
    var proportionOfRightView: CGFloat = 1
    var distanceOfLeftView: CGFloat = 50
    var distanceOfRightView: CGFloat = 50
    // 侧滑菜单黑色半透明遮罩层
    ...
    override func viewDidLoad() {
        ...
        if Common.screenWidth > 320 {
            ...
            proportionOfRightView = Common.screenWidth / 320
            distanceOfRightView += (Common.screenWidth - 320) * FullDistance / 2
        }
        ...
        centerOfRightViewAtBeginning = CGPointMake(( leftViewController.view.center.x + self.distanceOfRightView+280), self.rightViewController.view.center.y)        // 把侧滑菜单视图加入根容器
    ...
    func pan(recongnizer: UIPanGestureRecognizer) {
        ...
        let x = recongnizer.translationInView(self.view).x//滑动距离
        let trueDistance = distance + x // 实时距离
        let trueProportion = trueDistance / (Common.screenWidth*FullDistance)//实时比例
        
        // 如果 UIPanGestureRecognizer 结束，则激活自动停靠
        if recongnizer.state == UIGestureRecognizerState.Ended {

            if trueDistance > Common.screenWidth * (Proportion / 3.0) {
                showLeft()
            } else if trueDistance < Common.screenWidth * -(Proportion / 3.0) {
                showRight()
            } else {
                showHome()
            }
       ...
       //leftViewController.view.transform = CGAffineTransformScale(CGAffineTransformIdentity, pro, pro)
        ...
        if trueDistance > Common.screenWidth * (Proportion / 3.0){
        // 执行右视图动画
        let propor = 0.8 + (proportionOfRightView - 0.8) * trueProportion
        rightViewController.view.center = CGPointMake(centerOfRightViewAtBeginning.x - distanceOfRightView * trueProportion, centerOfRightViewAtBeginning.y - (proportionOfRightView - 1) * rightViewController.view.frame.height * trueProportion / 2 )
        //rightViewController.view.transform = CGAffineTransformScale(CGAffineTransformIdentity, propor, propor)
        }else{
        return//显示主视图
        }
    }
    ...
    func doTheAnimate(proportion: CGFloat, showWhat: String) {
        UIView.animateWithDuration(0.3, delay: 0, options: UIViewAnimationOptions.CurveEaseInOut, animations: { () -> Void in
            ...
            if showWhat == "right" {
                ...
                // 移动右侧菜单的中心
                self.rightViewController.view.center = CGPointMake((self.centerOfRightViewAtBeginning.x + self.distanceOfRightView-280), self.rightViewController.view.center.y)
                // 缩放右侧菜单
                self.rightViewController.view.transform = CGAffineTransformScale(CGAffineTransformIdentity, self.proportionOfRightView, self.proportionOfRightView)
            }
            // 改变黑色遮罩层的透明度，实现视差效果
